...
fn open(filename: &str) -> Result<Box<dyn BufRead>> {
    // Open file or stdin for reading, depending on command-line input
    match filename {
...
fn show_nonprinting_chars(line: String) -> String {
    ...
    let caret_encoding = HashMap::from([("\u{00}","^@"), ("\\u{01}","^A"), ("\\u{02}","^B"),
                     ("\\u{03}","^C"), ("\\u{04}","^D"), ("\\u{05}","^E"), ("\\u{06}","^F"),
                     ("\u{07}","^G"), ("\u{08}","^H"), ("\u{09}","^I"), ("\u{0a}","^J"),
    ...
    for c in line.chars() {
        ...
        if caret_encoding.contains_key(ce) {
            copy = copy.replace(c, caret_encoding[ce]);
        }
    }
    copy
}
