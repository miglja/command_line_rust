...
fn show_nonprinting_chars(line: String) -> String {
    ...
    let caret_encoding: HashMap<&str,&str> = HashMap::from([("\u{00}","^@",),
        ...
        ("\\u{09}","^I",), ("\\u{0a}","^J",), ("\\u{0b}","^K",), ("\\u{0c}","^L",),
        ("\\u{0d}","^M",), ("\\u{0e}","^N",), ("\\u{0f}","^O",), ("\\u{10}","^P",), 
        ("\\u{11}","^Q",), ("\\u{12}","^R",), ("\\u{13}","^S",), ("\\u{14}","^T",), 
        ("\\u{15}","^U",), ("\\u{16}","^V",), ("\\u{17}","^W",), ("\\u{18}","^X",),
        ("\\u{19}","^Y",), ("\\u{1a}","^Z",), ("\\u{1b}","^[",), ("\\u{1c}","^\\",),
        ("\\u{1d}","^]",), ("\\u{1e}","^^",), ("\\u{1f}","^_",), ("\\u{7f}","^?",)]);
